class PhysicsEngine {
    constructor(camera, domElement, worldManager) {
        this.camera = camera;
    ...
    update(delta) {
        ...
        const chunkSize = this.worldManager.getChunkSize();
        const blockSize = this.worldManager.getBlockSize();
        ...
        // Usa manager per leggere altezze JSON
        const getJSONTerrainHeight = (xPos, zPos) => {
            let i = Math.round((xPos / blockSize) + (chunkSize / 2));
            let j = Math.round((zPos / blockSize) + (chunkSize / 2));
            return this.worldManager.getTerrainHeight(i, j);
        };

        // COLLISIONI LATERALI (Muri base)
        let targetGroundHeightX = getJSONTerrainHeight(nextX + (Math.sign(-this.velocity.x) * colliderRadius), playerPos.z);
        let targetGroundHeightZ = getJSONTerrainHeight(playerPos.x, nextZ + (Math.sign(-this.velocity.z) * colliderRadius));
        ...
        // COLLISIONE COL PAVIMENTO SU JSON
        ...
        let floorHeight = this.worldManager.getTerrainHeight(currentI, currentJ);
        if (floorHeight === 0 && this.worldBounds > 0) {
            // In caso cada mappa esterna
            floorHeight = 5;
        }
